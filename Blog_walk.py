...
def get_totalwalks(num_walks_list,walk_length_list,Lmax,G,nodes,nodes_transition_inside,nodes_transition_boundary):
    ...
    in_nodes = node_degree[:math.ceil(len(ll)*0.7)]
    bd_nodes = node_degree[math.ceil(len(ll)*0.7):]
    walks = []
...
if __name__ == '__main__':
    # 读取边集文件，构造图，得到邻接矩阵
    G = nx.read_edgelist("BlogCatalog/Blog_network.txt", create_using=nx.DiGraph())
    G = G.to_undirected()
    adj_matrix = nx.adjacency_matrix(G).toarray()
    # 读取属性文件，得到属性矩阵
    Attr = np.loadtxt('BlogCatalog/Blog_feature.txt', dtype=int)
    ...
    # T = get_JaccardSim(G)
    # A = get_AttrSim(Attrs)
    T = np.loadtxt('TSim/BlogCatalog_T_DegreeSim.txt')
    A = np.loadtxt('ASim/BlogCatalog_A_Cosin.txt')
    ...
    # print(walks)
    print(len(walks))
    model = Word2Vec(walks, vector_size=512, window=5, min_count=0, sg=1, workers=8, epochs=4)
    X = model.wv.vectors
    # np.savetxt('D:\CodeData\RWSECD\Embedding\citeseer.emb',X)

    label = np.loadtxt('BlogCatalog/label.txt', dtype=int)
    real_com = label[:, -1]
    FscoreList = []
    AccList = []
    NMIList = []
    ARIList = []
    '''# emb = np.loadtxt('D:\CodeData\RWSECD\Embedding\cornel.emb')
    res = KMeans(n_clusters=6).fit_predict(X)
    # print(res)
    nodes = list(G.nodes)
    print(nodes)
    # nodes = sorted([int(node) for node in nodes])  # 将图的节点按从小到大排序，并用于后续将聚类结果与节点顺序一一对应
    # print(nodes)
    pred_com = []
    for node in nodes:  # 按networkx从小到大排序后的顺序遍历每个节点
        idx = model.wv.key_to_index[node]  # 获取这个节点在embedding 中的索引号也即是聚类结果列表中这个节点的索引号
        pred_com.append(res[idx])  # 获取这个节点的聚类结果并按照此节点在图中的顺序加入到社区发现结果中
    pred_com = np.array(pred_com)
    print(len(pred_com))
    metrics = Metrics(real_com, pred_com)
    Fscore, Accuracy, NMI, ARI = metrics.getFscAccNmiAri()
    print(Fscore, Accuracy, NMI, ARI)'''
    for i in tqdm(range(100)):
        ...
        pred_com = []
        for node in nodes:  # 按networkx从小到大排序后的顺序遍历每个节点
            idx = model.wv.key_to_index[str(node)]  # 获取这个节点在embedding 中的索引号也即是聚类结果列表中这个节点的索引号
            pred_com.append(res[idx])  # 获取这个节点的聚类结果并按照此节点在图中的顺序加入到社区发现结果中
        pred_com = np.array(pred_com)
        metrics = Metrics(real_com, pred_com)
        Fscore, Accuracy, NMI, ARI = metrics.getFscAccNmiAri()
