...
if __name__ == '__main__':
    # 读取边集文件，构造图，得到邻接矩阵
    G = nx.read_edgelist("citeseer/cite_network.txt", create_using=nx.DiGraph())
    G = G.to_undirected()
    adj_matrix = nx.adjacency_matrix(G).toarray()
    # 读取属性文件，得到属性矩阵
    Attr = np.loadtxt('citeseer/cite_feature.txt', dtype=int)
    ...
    # T = get_JaccardSim(G)
    # A = get_AttrSim(Attrs)
    T = np.loadtxt('TSim/citeseer_T_DegreeSim.txt')
    A = np.loadtxt('ASim/citeseer_A_Cosin.txt')
    ...
    label = np.loadtxt('citeseer/label.txt', dtype=int)
    real_com = label[:, -1]
    FscoreList = []
    AccList = []
    NMIList = []
